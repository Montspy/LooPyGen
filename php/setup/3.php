<?php
    ...
    if (!empty($traits) and $redirect !== "TRUE") { ?>
        ...
        <form enctype="multipart/form-data" onSubmit="return rarityCheck(this);" method="post" action="/setup/3?collection=<?php echo $collection; ?>">
            <?php $t = 0;
            while ($s <= $traits['trait_count']) {
                if ($t == 0 and $traits['background_color'] === true) { // Background color layer
                    unset($layer);
                    $weights_empty = false;
                    ...
                    <?php $v = 1; while ($v <= $traits["image_layers"][$s]['variations']) { // Background color variations
                        $trait_var = $s . "_" . $v;
                        unset($var_name);
                    ...
                    <h3 hidden class="error" id="trait<?php echo $s; ?>_error">No error</h3>
                    <?php $v = 1; while ($v <= $traits['image_layers'][$t]['variations']) { // Image layers variations
                        $trait_var = $s . "_" . $v;
                        unset($var_name);
                        if (isset($layer) and isset($layer['weights'][$v - 1])) {
        ...
        while ($s <= $traits['trait_count']) {
            ...
            if ($t == 0 and $traits['background_color'] === true) {
                ...
                $traits["image_layers"][$t]['weights_total'] = (float)$_POST["trait${s}_empty_rarity"] + array_sum($traits["image_layers"][$t]['weights']); // Should be 100
            } else {
                $v = 1;
                ...
                while ($v <= $traits['image_layers'][$t]['variations']) {
                    ...
                    $traits["image_layers"][$t]['filenames'][$var_name] = $filename;    // Add variation filename to filename array (with variation name as key)
                    array_push($traits["image_layers"][$t]['weights'], $var_weight);    // Add weights to weight array
                    $v = $v + 1;
                }
                $traits["image_layers"][$t]['weights_total'] = (float)$_POST["trait${s}_empty_rarity"] + array_sum($traits["image_layers"][$t]['weights']); // Should be 100
            }
        ...
        file_put_contents("./collections/${collection}/config/traits.json", $traits_json);
        Redirect("/setup/finish?collection=${collection}", false);
    } else {
